...
resource "azurerm_network_security_rule" "mysgrule" {
    count = 2 
    name = "mysngrule-${count.index}"
    network_security_group_name = azurerm_network_security_group.mysg[count.index].name
    resource_group_name = azurerm_resource_group.rg1.name
...
resource "azurerm_network_interface_security_group_association" "myngsgassoc" {
  ...
  network_interface_id = azurerm_network_interface.munvinterface[count.index].id
  network_security_group_id = azurerm_network_security_group.mysg[count.index].id
